...
<div class="container w-800">
	...
	<div class="cell right">
		<h2>
			<a class="link" href="insert"><i class="fa-solid fa-plus"></i>
				게시글 작성</a>
		</h2>
	</div>
	<div class="cell">
			...
			<tbody align="center">
				<c:forEach var="inquiryDto" items="${list}">
					<tr>
						...
						<td class="left">
							<%-- 제목 출력 --%> <a class="link"
							href="detail?inquiryNo=${inquiryDto.inquiryNo}">
								${inquiryDto.inquiryTitle} </a>
						</td>
						<td>${inquiryDto.inquiryWriterStr}</td>
						<%-- dto 에서 가상의 메소드 하나 만들어주기 --%>
						<td>${inquiryDto.inquiryWtimeStr}</td>
						<%-- dto 에서 가상의 메소드 하나 만들어주기 --%>
	...
	<div class="cell left">
		...
		<form action="list" method="get">
			<select name="column" class="tool">
				<option value="inquiry_title"
					${param.column == 'inquiry_title' ? 'selected' : ''}>제목</option>
				<option value="inquiry_writer"
					${param.column == 'inquiry_writer' ? 'selected' : ''}>작성자</option>
				<option value="inquiry_content"
					${param.column == 'inquiry_content' ? 'selected' : ''}>내용</option>
